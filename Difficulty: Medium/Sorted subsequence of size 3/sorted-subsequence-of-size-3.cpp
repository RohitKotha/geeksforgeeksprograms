...
class Solution {
  public:
    vector<int> find3Numbers(vector<int> &arr) {
        int n = arr.size();
        vector<pair<int,int>> left_min(n);
        left_min[0] = {arr[0],0};
        for(int i = 1; i < n; i++)
        {
            left_min[i] = left_min[i-1];
            if(arr[i] < left_min[i-1].first)
            {
                left_min[i] = {arr[i],i};
            }
        }
        vector<int> ans;
        pair<int,int> rmax = {arr[n-1],n-1};
        for(int i = n-2; i >= 1; i--)
        {
            if(rmax.first > arr[i] && arr[i] > left_min[i-1].first)
            {
                return {arr[left_min[i-1].second],arr[i],arr[rmax.second]};
            }
            if(arr[i] > rmax.first)
            {
                rmax = {arr[i],i};
